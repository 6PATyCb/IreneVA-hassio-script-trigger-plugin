...
- В списке скриптов плагин ищет совпадения фразы и алиаса скрипта (HA позволяет давать названия скриптам на кириллице). Совпадение может быть как полным (в файле core.json, параметр "fuzzyThreshold": 1.0), так и частичным (например, "fuzzyThreshold": 0.8).
